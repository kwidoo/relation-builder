...
export default defineComponent({
  ...
  setup() {
    ...
    const { MORPH_MANY, morphManyStub, morphManyScopes } = useConstants();
    let output = ref('');
    ...
    const generateRelation = async () => {
      ...
      const initialStub = await morphManyStub();
      let finalOutput = initialStub.replace(/{{\s*scopes\s*}}/g, withScopes.value ? await morphManyScopes() : '');
      ...
      if (asTrait.value) {
        ...
        finalOutput = trait(namespace.value, MORPH_MANY(relatedModel.value), finalOutput);
      }
    ...
    return {
      ...
      asTrait,
      withScopes,
      filename: MORPH_MANY(relatedModel.value) // Adjust filename to reflect the relationship
    };
  },
});
</script>
