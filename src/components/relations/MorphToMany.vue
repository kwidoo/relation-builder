...
export default defineComponent({
  name: 'MorphToManyRelationBuilder',
  components: {
    ModelInput,
    CodeComponent,
    OptionsBar,
  ...
  setup() {
    ...
    const { MORPH_TO_MANY, morphToManyStub, morphToManyScopes } = useConstants();
    let output = ref('');

    const relatedModel = ref('Tag');
    const relatedType = ref('taggable'); 
    ...
    const generateRelation = async () => {
      ...
      const initialStub = await morphToManyStub();
      let finalOutput = initialStub.replace(/{{\s*scopes\s*}}/g, withScopes.value ? await morphToManyScopes() : '');

      finalOutput = finalOutput
        .replace(/{{\s*methodName\s*}}/g, camelCase(relatedModel.value) + 's')
        .replace(/{{\s*relatedModel\s*}}/g, relatedModel.value)
        .replace(/{{\s*relatedType\s*}}/g, relatedType.value);
      
      if (asTrait.value) {
        const { trait } = await useAsTrait();
        finalOutput = trait(namespace.value, MORPH_TO_MANY(relatedModel.value), finalOutput);
      }
    ...
    return {
      ...
      asTrait,
      withScopes,
      filename: MORPH_TO_MANY(relatedModel.value) // Adjust filename to reflect the relationship
    };
  },
});
</script>
